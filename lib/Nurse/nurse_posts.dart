...
/// - Like posts
/// - Add comments
class NursePosts extends StatefulWidget {
  const NursePosts({super.key});

  @override
  State<NursePosts> createState() => _NursePostsState();
}

class _NursePostsState extends State<NursePosts> {
  ...
  Future<void> _loadPosts() async {
    try {
      ...
      for (var post in posts) {
        print('[Nurse Posts] Post ${post.id}: image="${post.image}", image is null: ${post.image == null}, image isEmpty: ${post.image?.isEmpty ?? true}');
      }
      ...
      print('[Nurse Posts] Error loading posts: $e');
      if (mounted) {
  ...
  Future<void> _likePost(String postId) async {
    ...
    if (_isLiking[postId] == true || _hasLiked[postId] == true) {
      print('[Nurse Posts] User already liked this post or is currently liking');
      return;
    }
    ...
    try {
      ...
      print('[Nurse Posts] Error liking post: $e');
      if (mounted) {
  ...
  Future<void> _addComment(String postId) async {
    ...
    try {
      ...
      print('[Nurse Posts] Error adding comment: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          if (post.image != null && post.image!.isNotEmpty) ...[
            Builder(
              builder: (context) {
                ...
                print('[Nurse Posts] Rendering image for post ${post.id}: "$imageUrl"');
                
                // Check if it's a direct HTTP/HTTPS URL
                final isHttpUrl = imageUrl.startsWith('http://') || imageUrl.startsWith('https://');
                
                // Check if it's a Firebase Storage path (gs:// or storage path)
                final isStoragePath = imageUrl.startsWith('gs://') || 
                                     (!isHttpUrl && !imageUrl.contains('://') && imageUrl.isNotEmpty);
                  ...
                  return FutureBuilder<String?>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError || !snapshot.hasData || snapshot.data == null) {
                        print('[Nurse Posts] Error getting download URL: ${snapshot.error}');
                        return _buildImageError(imageUrl);
                      }
                      
                      final downloadUrl = snapshot.data!;
                      print('[Nurse Posts] Got download URL: $downloadUrl');
                      return _buildNetworkImage(downloadUrl, imageUrl);
                    },
                  );
                } else {
                  // Invalid format
                  print('[Nurse Posts] Image is not a valid URL or path: $imageUrl');
                  return _buildImageError(imageUrl);
                }
              },
            ),
                        ...
                        _isLiking[post.id] == true
                            ...
                            : Icon(
                                _hasLiked[post.id] == true
                                    ...
                                    : Icons.favorite_border, // Empty heart if not liked
                                size: 20,
                                color: _hasLiked[post.id] == true
                                    ? Colors.red // Red if liked
                                    : theme.colorScheme.primary, // Primary color if not liked
                              ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...post.comments.map((comment) {
                    ...
                    final authorName = comment['authorName'] as String? ?? 'مستخدم';
        ...
        child: Image.network(
          ...
          loadingBuilder: (context, child, loadingProgress) {
            if (loadingProgress == null) {
              print('[Nurse Posts] Image loaded successfully: $imageUrl');
              return child;
            }
            return Container(
              height: 200,
          ...
          errorBuilder: (context, error, stackTrace) {
            print('[Nurse Posts] Error loading image: $error, URL: $imageUrl');
    ...
    try {
      ...
      if (path.startsWith('gs://')) {
        ...
        if (parts.length > 3) {
          ...
          print('[Nurse Posts] Invalid gs:// URL format: $path');
          return null;
        }
      }
      
      print('[Nurse Posts] Getting download URL for storage path: $storagePath');
      final ref = FirebaseStorage.instance.ref().child(storagePath);
      final url = await ref.getDownloadURL();
      print('[Nurse Posts] Download URL obtained: $url');
      return url;
    } catch (e) {
      print('[Nurse Posts] Error getting download URL from Firebase Storage: $e');
      // Try as direct path if gs:// conversion failed
      try {
        final ref = FirebaseStorage.instance.ref().child(path);
        final url = await ref.getDownloadURL();
        print('[Nurse Posts] Download URL obtained (direct path): $url');
        return url;
      } catch (e2) {
        print('[Nurse Posts] Error with direct path: $e2');
        return null;
      }
    }
  }
